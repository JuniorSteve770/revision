...
const Results = ({ scores }) => {
  ...
  return (
    <div className="results">
      ...
      {totalScore >= 3 ? (
        <h3 className="success">🚀 Prêt pour les questions de conception en entretien !</h3>
      ) : (
        <p className="fail">📚 Relisez les slides pour consolider ces fondamentaux.</p>
      )}
    </div>
  );
};
      ...
      {showResult ? <Results scores={scores} /> : (
        <div>
          <h4 className="subtitle" style={{ fontSize: '10px', margin: '0 0 6px 0' }}>
            POO / SOLID / ACID / Patterns 🔹 {level === "basic" ? `Slide ${currentSlide + 1}/${basicSlides.length}` : `QCM ${level.toUpperCase()}`}
          </h4>
          {level === "basic" ? (
